    ...
    <article class="wiki-profile ${item.kind} ${item.gender||item.kind}">
      ...
      <div class="wiki-layout">
        ...
        <aside class="profile-infobox${portraitUrl?"":" no-portrait"}">
          ...
          ${statuses.length?`<section><h3>Status history</h3>${proseList(statuses.map(event=>`Chapter ${event.chapter} — ${event.value}`))}</section>`:""}
        </aside>
      </div>
    </article>`;
...
function renderAdmin(){
  ...
  const sourcesField=$("#chapter-sources-text");if(sourcesField&&document.activeElement!==sourcesField)sourcesField.value=chapterSourcesToText(data.chapterSources);
...
function updateEventHelp(){
  ...
  $("#event-value-label").textContent=type==="display_name"?"New public display name":type==="identity_parent"?"Identity relationship":type==="alias"?"New alias":type==="cultivation"?"Equivalent path name (optional)":type==="status"?"New status":type==="relationship"?"Relationship (friendly, neutral, or hostile)":isOrgLocation?"Organization location role":isResidence?"Residence type":"Role or title (optional)";
...
function buildEventRecord(formElement,id="ev-"+crypto.randomUUID()){
  ...
  if(["alias","display_name","identity_parent","status","relationship","organization_location","residency"].includes(type)&&!value){toast(`Enter the ${$("#event-value-label").textContent.toLowerCase()}`);return null;}
