...
const sampleData = {
  ...
  entities: [
    ...
    { id: "q-winter", kind: "quest", issuer: "inn-system", name: "Survive the Long Frost", intro: 18, countsAlone: true, questBadge: "Urgent", rating: "S", timeLimit: "Ninety days", failure: "Authority falls by one and the Hearthkeeper System is recalled.", description: "Keep every guest alive until the thaw." },
    ...
    { id: "caravan-hands", kind: "character", name: "Four caravan hands", gender: "unknown", mentioned: null, appeared: 28, description: "The crew who bring the Stonevale caravan through — one figure until the story picks somebody out of it." },
    { id: "mira", kind: "character", name: "Mira", gender: "female", mentioned: null, appeared: 29, description: "The hand who speaks up, and the only one of the four the story keeps." },
  ...
  events: [
    ...
    { id: "e13", chapter: 18, type: "meeting", source: "lex", target: "luthor", personas: { lex: "the Innkeeper" }, description: "Lex and Luthor meet — though what Luthor meets is the Innkeeper." },
    ...
    { id: "crowd-1", chapter: 28, order: 8, type: "appearance", source: "caravan-hands", creationManaged: true, description: "Four caravan hands appear for the first time." },
    { id: "crowd-2", chapter: 29, order: 1, type: "appearance", source: "mira", creationManaged: true, description: "Mira appears for the first time." },
    { id: "crowd-3", chapter: 29, order: 2, type: "part_of", source: "mira", target: "caravan-hands", action: "add", description: "Mira is one of the four caravan hands." },
    { id: "crowd-4", chapter: 29, order: 3, type: "conversation", source: "mira", target: null, characters: ["mira", "lex"], location: "stonevale", description: "One of the hands speaks up about the pass, and turns out to be called Mira." },
    { id: "crowd-5", chapter: 31, order: 1, type: "departure", source: "caravan-hands", action: "add", withInside: false, ghost: true, value: "The caravan is through and the crew go their own way", description: "Four caravan hands are gone from here on: the caravan is through and the crew go their own way." },
    ...
    { id: "e33b", chapter: 45, order: 5, type: "cultivation", source: "eclipse", level: 7, atLeast: true, description: "Nobody in the Garden can read Eclipse: all that is certain is that she is at least an Earth Immortal." },
    ...
    { id: "corpse-3", chapter: 37, order: 1, type: "departure", source: "halden", action: "add", value: "Buried in Stonevale; the story never returns to him", ghost: true, description: "Halden is gone from here on: buried in Stonevale; the story never returns to him." },
    ...
    { id: "qa-7", chapter: 9, order: 1, type: "quest_end", source: "q-stock", action: "complete", rewards: ["40 contribution points", "Cellar Key"], performance: "B", description: "Quest complete. The cellar is full, a day early." },
    { id: "qa-8", chapter: 11, order: 1, type: "quest_end", source: "q-cook", action: "complete", rewards: ["Mary's service contract", "Kitchen rights", "Authority +1"], performance: "S", description: "Quest complete. Mary takes the kitchen and stays." },
    ...
    { id: "qn-7", chapter: 32, order: 1, type: "quest_end", source: "q-anomaly", action: "complete", rewards: ["Realm Seed", "Warden rank raised to Destiny", "3,000 contribution points"], performance: "SSS+", description: "Quest complete. Anomaly source: a perpetual inter-realm wormhole." },
    ...
    { id: "qj-3", chapter: 36, order: 1, type: "quest_end", source: "q-caravan", action: "complete", rewards: ["Split by contribution"], performance: "A+", description: "Quest complete. The caravan reaches Stonevale intact." },
    { id: "qj-4", chapter: 36, order: 2, type: "quest_contribution", source: "q-caravan", target: "lex", value: "Held the rear against the frost", rewards: ["Frostbound Cloak", "600 contribution points"], performance: "A+", description: "Lex contributed the greater share." },
    { id: "qj-5", chapter: 36, order: 3, type: "quest_contribution", source: "q-caravan", target: "vane", value: "Broke the road ahead", rewards: ["300 contribution points"], performance: "B+", description: "Vane contributed the lesser share." },
    ...
    { id: "talk-1", chapter: 22, order: 5, type: "conversation", source: "lex", characters: ["lex","mary","gerald"], location: "inn-lobby", mentions: ["stonevale","jotun"], relation: "friendly", holds: 3, description: "Lex, Mary, and Gerald settle how the Inn will run its front of house — and what to do if the Jotun Empire ever sends word from Stonevale." },
...
let expandedLocations = new Set(), expandedSystems = new Set(), expandedCrowds = new Set();
let lastLocationView = null, lastSystemView = null, lastCrowdView = null;
...
// The slider either steps a chapter at a time, gathering that chapter's actions into one stop,
// or steps every action in turn. Which one is the reader's choice, kept between visits.
let sliderStepsEvents=false;
  ...
  <div class="app">
    <header class="topbar">
      ...
      <div class="chapter-ref-toggle-wrap">
        ...
        <button type="button" id="toggle-slider-mode" class="chapter-ref-toggle" aria-pressed="false" title="Step the slider a chapter at a time, or one action at a time"><span aria-hidden="true">⏱</span> <span id="slider-mode-label">By chapter</span></button>
      ...
      <section id="graph-view" class="view active">
        <div class="graph-page">
          ...
          <div class="graph-layout" id="graph-layout">
            ...
            <aside class="side">
              ...
              <section class="side-card mobile-legend" data-panel-content="legend" aria-label="Mobile graph legend">
                ...
                <span><i class="line-key friendly"></i>Friendly</span><span><i class="line-key hostile"></i>Hostile</span><span><i class="line-key neutral"></i>Neutral / awareness</span><span><i class="line-key clone"></i>Clone / avatar</span><span><i class="line-key member"></i>Membership</span><span><i class="line-key crowd"></i>One of them</span><span><i class="line-key location"></i>Travel / activity</span><span><i class="line-key residence"></i>Residence</span><span><i class="line-key hierarchy"></i>Inside location</span><span><i class="line-key organization-location"></i>Organization place</span>
          ...
          <div class="legend desktop-legend" aria-label="Graph legend">
            <span><i class="dot female"></i>Female</span><span><i class="dot male"></i>Male</span><span><i class="hex"></i>Organization — opens too, if it holds places</span><span><i class="pin"></i>Place — click twice to open</span><span><i class="pin held"></i>Place inside an organization</span><span><i class="gem"></i>System — click to open its reach</span><span><i class="line-key conversation"></i>Conversation</span><span><i class="line-key mention"></i>Spoken of</span>
            <span><i class="line-key friendly"></i>Friendly</span><span><i class="line-key hostile"></i>Hostile</span><span><i class="line-key neutral"></i>Neutral / awareness arrow</span><span><i class="line-key clone"></i>Clone / avatar</span><span><i class="line-key member"></i>Membership</span><span><i class="line-key crowd"></i>One of them</span><span><i class="line-key location"></i>Travel / activity</span><span><i class="line-key residence"></i>Residence</span><span><i class="line-key hierarchy"></i>Inside location</span><span><i class="line-key organization-location"></i>Organization place</span>
            <span><i class="ring mentioned"></i>Mentioned only</span><span><i class="ring unknown"></i>Unknown status</span><span><i class="ring"></i>Alive</span><span><i class="ring dead"></i>Dead</span><span><i class="diamond"></i>Alias count</span><span><i class="corona-key"></i>Cultivation level</span>
          </div>
        </div>
      </section>
...
2 | https://www.webnovel.com/book/.../chapter-title_789012"></textarea></label>
          ...
          <div class="admin-grid">
            <form id="entity-form" class="admin-card"><div class="admin-card-heading"><h2 id="entity-form-title">Create character, organization, or location</h2><div class="manage-row"><input id="manage-entity" list="admin-entity-options" placeholder="Load an existing identity"><button class="button ghost" id="load-entity" type="button">Load</button></div></div><p id="creation-edit-note" class="event-edit-mode-note" hidden><strong>Editing a creation-generated row.</strong> These facts were made by this identity form, so saving here updates the original linked row instead of creating a chapter event.</p><input name="editingId" type="hidden"><input name="editingCreationEventId" type="hidden"><div class="form-grid">
              ...
              <label class="field checkbox-field" id="entity-quest-alone-field" hidden><input type="checkbox" name="countsAlone" /><span>Counted as its own active quest</span><small class="field-note">Off by default: a quest that is part of a larger one is counted with it, so a chain of three reads as one running quest. Turn this on where the story treats the part as a quest in its own right.</small></label>
              ...
              <label class="field" id="entity-quest-reward-rank-field" hidden><span>Quest rating (optional)</span><input name="rating" list="system-grade-options" maxlength="9" placeholder="SSS+" /><small class="field-note">How the mission itself is rated. Write it exactly as the story writes it — E through A+, S through SSS+, or a named rank such as Destiny, Divine, or Chaos. Most quests do not say until they are finished, so leave this blank and fill the rank in on the completion action instead.</small></label>
              ...
              <label class="field" id="entity-quest-rewards-field" hidden><span>Rewards (optional)</span><textarea name="rewards" rows="2" placeholder="200 contribution points&#10;Hearth token"></textarea><small class="field-note">One per line, or separated by commas. A reward can carry a wiki link. A sub-quest can carry its own rewards, and many quests carry none.</small></label>
              ...
              <label class="field" id="entity-mentioned-field"><span id="entity-mentioned-label">First mentioned chapter (optional)</span><input name="mentioned" type="number" min="1" placeholder="Leave blank if they appear directly"></label>
              ...
              <label class="field checkbox-field" id="entity-initial-at-least-field"><input type="checkbox" name="initialAtLeast" /><span>Only a floor — at least this, possibly higher</span></label><label class="field" id="entity-initial-cultivation-alias-field"><span>Equivalent path title (optional)</span><input name="initialCultivationAlias" placeholder="E.g. Earthen Deity"><small class="field-note">Synchronized to the canonical tier selected above.</small></label>
            ...
            <form id="event-form" class="admin-card"><h2 id="event-form-title">Add chapter changes</h2><p id="event-edit-mode-note" class="event-edit-mode-note" hidden><strong>Editing this exact saved event.</strong> Saving replaces only the row you clicked; it does not create a new event or edit the identity form.</p><input name="editingId" type="hidden"><div class="form-grid">
              ...
              <label class="field" id="event-location-field"><span id="event-location-label">Where this happened (optional)</span><input name="location" list="location-options" placeholder="Type a place or an organization"><datalist id="location-options"></datalist><small class="field-note" id="event-location-note">A place, or the organization it happened inside — an office, a sect hall, a guild.</small></label><label class="field" id="event-branch-field" hidden><span>Which branch of it (optional)</span><input name="branch" list="branch-options" placeholder="Leave empty for the original branch"><datalist id="branch-options"></datalist><small class="field-note" id="event-branch-note">Only when one branch takes part rather than the whole organization. Empty means the original branch — the first place it opened.</small></label>
              <label class="field" id="event-target-field"><span id="event-target-label">Second character</span><input name="target" list="admin-entity-options"></label><label class="field" id="event-authority-field"><span>New authority</span><input name="authority" list="system-unknown-options" inputmode="numeric" maxlength="12" placeholder="8" /><small class="field-note">Leave blank to keep the authority it already has. Write unknown to take it away entirely.</small></label><label class="field" id="event-grade-field"><span>New grade</span><input name="grade" list="system-grade-options" maxlength="9" placeholder="S+" /><small class="field-note">Leave blank to keep the grade it already has. Write unknown to take it away entirely.</small></label><label class="field" id="event-progress-field"><span>Progress %</span><input name="progress" inputmode="numeric" maxlength="3" placeholder="40" /><small class="field-note">Nought to a hundred. A quest made of parts, with no figure of its own, takes the average of its parts.</small></label><label class="field" id="event-note-kind-field"><span>Kind of note</span><select name="noteKind"><option value="Update">Quest update</option><option value="Hint">Quest hint</option><option value="Remark">Remark</option><option value="Notification">New notification</option><option value="Objective">Added objective</option></select><small class="field-note">A quest's terms can grow as the story turns — new objectives, hints, and the system's own remarks all belong here rather than in a second quest.</small></label><label class="field span-2" id="event-rewards-field"><span>Rewards (optional)</span><textarea name="rewards" rows="2" placeholder="Realm Seed&#10;200 contribution points&#10;Authority +1"></textarea><small class="field-note">One per line, or separated by commas — a quest can pay several things at once, and a reward can be an item, a quantity, a rank, or anything else the story gives.</small></label><label class="field" id="event-performance-field"><span>Performance grade (optional)</span><input name="performance" list="system-grade-options" maxlength="9" placeholder="A" /><small class="field-note">How the host performed, which is generally what the reward is scaled to.</small></label><label class="field checkbox-field" id="event-with-inside-field"><input type="checkbox" name="withInside" checked /><span>Take everyone standing inside it too</span><small class="field-note">Untick to leave them behind: the figure goes, and whoever was picked out of it stays on the graph in their own right.</small></label><label class="field checkbox-field" id="event-at-least-field"><input type="checkbox" name="atLeast" /><span>Only a floor — at least this, possibly higher</span><small class="field-note">For a rank the story never states outright: too high for the reader to see, or simply above a tier. Everything above it is then drawn as unread rather than as unattained.</small></label><label class="field checkbox-field" id="event-ghost-field"><input type="checkbox" name="ghost" /><span>Ghost action — change the world without appearing in the list</span></label><label class="field span-2" id="event-mentions-field"><span>Also spoken of (optional)</span><input name="mentions" list="admin-entity-options" placeholder="Jotun Empire, Stonevale" /><small class="field-note">People, places or groups this action only refers to. They are drawn as spoken of — a thin line to the one who spoke of them — not as being there.</small></label><label class="field" id="event-holds-field"><span>Lasts for (optional)</span><input name="holds" type="text" inputmode="numeric" maxlength="3" placeholder="3" /><small class="field-note">How many actions after its own this keeps showing on the graph, for something that is still going on while other things happen. Leave it empty for an action that is over as soon as it lands.</small></label><label class="field" id="event-relation-field"><span>How it leaves them</span><select name="relation"><option value="">Unchanged — neutral until something says otherwise</option><option value="neutral">Neutral</option><option value="friendly">Friendly</option><option value="hostile">Hostile</option></select><small class="field-note">Everyone in it ends up standing this way towards everyone else in it.</small></label><label class="field" id="event-persona-field"><span>Acting as (optional)</span><input name="persona" list="persona-options" maxlength="160" placeholder="Lex as the Innkeeper" /><datalist id="persona-options"></datalist><small class="field-note">The face the world sees here. Write <code>Name as Face</code> — or just the face, which then belongs to the subject. Several are separated by commas, because each side of a meeting may be seen as somebody else.</small></label><label class="field" id="event-characters-field"><span id="event-characters-label">Everyone in the conversation</span><input name="characters" list="admin-entity-options" placeholder="Lex, Mary, Gerald" /><small class="field-note" id="event-characters-note">Separate names with commas. One action covers the whole conversation, however many are in it.</small></label>
              <label class="field" id="event-value-field"><span id="event-value-label">Value</span><input name="value"><datalist id="location-role-options"><option value="Headquarters"></option><option value="Branch"></option><option value="Base"></option><option value="Territory"></option><option value="Outpost"></option></datalist><datalist id="identity-relation-options"><option value="Clone"></option><option value="Avatar"></option><option value="Persona"></option><option value="Public face"></option><option value="Puppet"></option><option value="Fragment"></option></datalist><datalist id="residence-role-options"><option value="Home"></option><option value="Permanent resident"></option><option value="Resident staff"></option><option value="Long-term guest"></option><option value="Camp"></option><option value="Domain"></option></datalist></label>
...
// A floor is not a rank: "Golden Core or above" is the whole of what was said.
function realmText(state,realm=state?.realm){return state?.atLeast?`${realm} or above`:realm;}
...
function loadLocalData() {
  ...
  try {
    ...
    if((migrated.schemaVersion||1)<16){
      const isBundledDemo=["lex","eclipse","inn-lobby"].every(id=>migrated.entities.some(item=>item.id===id));
      // One part of the chain is a quest in its own right, so the demo shows both sides of the
      // choice rather than only the default.
    ...
    if((migrated.schemaVersion||1)<20){
      // A reward is a thing the story hands over, and a quest can pay several. It has no rank of
      // its own — only the mission is rated — so what was stored as a reward rank is kept as one
      // more reward on the event, and a quest's own stated rank becomes its rating.
      (migrated.events||[]).forEach(event=>{
        if(!["quest_end","quest_contribution"].includes(event.type))return;
        const rewards=[...(Array.isArray(event.rewards)?event.rewards:[]),
          ...(event.type==="quest_end"&&event.value?String(event.value).split(",").map(part=>part.trim()).filter(Boolean):[]),
          ...(event.rewardRank?[`Rank ${event.rewardRank}`]:[])];
        if(rewards.length)event.rewards=rewards;
        if(event.type==="quest_end")delete event.value;
        delete event.rewardRank;
      });
      (migrated.entities||[]).forEach(item=>{if(item.kind!=="quest")return;if(item.rewardRank&&!item.rating)item.rating=item.rewardRank;delete item.rewardRank;});
      const isBundledDemo=["lex","eclipse","inn-lobby"].every(id=>migrated.entities.some(item=>item.id===id));
      if(isBundledDemo){
        sampleData.events.filter(event=>["quest_end","quest_contribution"].includes(event.type)).forEach(sample=>{
          const stored=(migrated.events||[]).find(event=>event.id===sample.id);
          if(stored){stored.rewards=deepClone(sample.rewards||[]);delete stored.value;if(sample.value)stored.value=sample.value;}
        });
        sampleData.entities.filter(item=>item.kind==="quest"&&item.rating).forEach(sample=>{
          const stored=migrated.entities.find(item=>item.id===sample.id);
          if(stored&&!stored.rating)stored.rating=sample.rating;
        });
      }
      migrated.schemaVersion=20;
    }
    if((migrated.schemaVersion||1)<21){
      const isBundledDemo=["lex","eclipse","inn-lobby"].every(id=>migrated.entities.some(item=>item.id===id));
      // An action can now say which face was worn for it, and the demo shows one being worn.
      if(isBundledDemo){const meeting=(migrated.events||[]).find(event=>event.id==="e13");
        if(meeting&&!meeting.persona&&!meeting.personas){const sample=sampleData.events.find(event=>event.id==="e13");meeting.personas=deepClone(sample.personas);meeting.description=sample.description;}}
      migrated.schemaVersion=21;
    }
    if((migrated.schemaVersion||1)<22){
      const isBundledDemo=["lex","eclipse","inn-lobby"].every(id=>migrated.entities.some(item=>item.id===id));
      // An action can now name what it only speaks of, and the demo speaks of two such things.
      if(isBundledDemo){const talk=(migrated.events||[]).find(event=>event.id==="talk-1");
        if(talk&&!talk.mentions){const sample=sampleData.events.find(event=>event.id==="talk-1");talk.mentions=deepClone(sample.mentions);talk.description=sample.description;}}
      migrated.schemaVersion=22;
    }
    if((migrated.schemaVersion||1)<23){
      // A face belongs to somebody in particular — in a conversation each side may be seen as
      // somebody else — so what was written for the whole action is moved onto its subject.
      (migrated.events||[]).forEach(event=>{
        if(!event.persona)return;
        if(event.source)event.personas={...(event.personas||{}),[event.source]:event.persona};
        delete event.persona;
      });
      migrated.schemaVersion=23;
    }
    if((migrated.schemaVersion||1)<24){
      const isBundledDemo=["lex","eclipse","inn-lobby"].every(id=>migrated.entities.some(item=>item.id===id));
      // A conversation can settle where people stand, and can go on while other things happen.
      if(isBundledDemo){const talk=(migrated.events||[]).find(event=>event.id==="talk-1"),sample=sampleData.events.find(event=>event.id==="talk-1");
        if(talk&&sample&&!talk.relation){talk.relation=sample.relation;talk.holds=sample.holds;}}
...
function richInline(value){
  const text=String(value??""),pattern=/\[\[(cite|mark):(\d+)\]\]|\[\[\/(?:cite|mark)\]\]|\[\[([^\]|[]+?)\|([^\]|[]+?)(?:\|([^\]|[]+?))?\]\]|\[\[(\d+)\]\]/g;
  ...
  while((match=pattern.exec(text))){
    ...
    if(match[1]!==undefined){
      ...
      const chapter=validChapter(match[2]);
      if(chapter){html+=`<span class="prose-chapter-ref sentence-cite${match[1]==="mark"?" perma-mark":""}" data-chapter="${chapter}">`;openChapter=chapter;}
      else html+=escapeHtml(match[0]);
      lastIndex=pattern.lastIndex;continue;
    }
    if(match[0]==="[[/cite]]"||match[0]==="[[/mark]]"){
      if(openChapter!==null)closeZone();else html+=escapeHtml(match[0]);
      lastIndex=pattern.lastIndex;continue;
    }
    if(match[6]!==undefined){
      const chapter=validChapter(match[6]);
    ...
    if(match[5]!==undefined){
      let urlPart=match[4].trim(),chapterPart=match[5].trim();
      ...
      const target=match[4].trim(),chapter=validChapter(target);
      if(chapter){
...
// A place or a group does not walk on the way a person does: the row that introduces it is its
// appearance, and until there is one it is only ever spoken of. One written down before any row
// existed for it carries its own introduction chapter instead — unless all it has is a mention.
function firstAppearance(item){
  const rows=data.events.filter(event=>event.source===item?.id&&(["appearance","corpse_appearance"].includes(event.type)||event.identityIntro===true)).map(event=>event.chapter),
    declared=item&&item.kind!=="character"&&!validChapter(item.mentioned)?item.intro:undefined;
  return earliestChapter([item?.appeared,declared,...rows]);
}
...
// A face is worn by somebody in particular. In a conversation each side may be seen as somebody
// else — Bastet is not talking to Lex Williams, she is talking to the Innkeeper — so the faces
// are kept per identity rather than one to the whole action. A bare face belongs to the subject,
// which is what the older shape stored.
function personasOf(event){
  const worn=new Map();
  if(event?.persona&&event.source)worn.set(event.source,String(event.persona));
  Object.entries(event?.personas||{}).forEach(([id,face])=>{if(id&&face)worn.set(id,String(face));});
  return worn;
}
...
// Not everything is over the moment the next thing starts. A conversation runs on while somebody
// rides for the door; a siege lasts a dozen actions. "Lasts for" is how many actions after its
// own an action keeps showing on the graph, and until that count runs out it is still happening.
function heldEvents(){
  if(currentActionIndex<=0)return [];
  return volumeActions().map((event,index)=>({event,at:index+1}))
    .filter(item=>item.at<currentActionIndex&&item.at+(Number(item.event.holds)||0)>=currentActionIndex)
    .map(item=>item.event);
}
function holdsLeft(event){
  const at=volumeActions().findIndex(item=>item.id===event.id)+1;
  // An action the reader has not reached is not going on yet, however long it will last.
  return at>0&&at<=currentActionIndex?Math.max(0,at+(Number(event.holds)||0)-currentActionIndex):0;
}
function stillHolding(event){
  const holds=Number(event.holds)||0;if(holds<1)return false;
  const at=volumeActions().findIndex(item=>item.id===event.id)+1;
  return at>0&&currentActionIndex>=at&&currentActionIndex<=at+holds;
}
...
function questSearchText(id){
  ...
  return [quest.name,quest.issuer?entity(quest.issuer)?.name:"",quest.description,quest.rewards?.length?`rewards ${quest.rewards.join(" ")}`:"",quest.rating?`rating ${quest.rating}`:"",quest.achievements?`achievements ${quest.achievements}`:"",quest.failure?`failure punishment ${quest.failure}`:"",quest.timeLimit?`time limit ${quest.timeLimit}`:"",quest.rewardRank?`rank ${quest.rewardRank}`:"",quest.questBadge||""].filter(Boolean).join(" ").toLowerCase();
}
function actionHaystack(event){
  const store=searchStore(),cached=store.haystacks.get(event.id);
  if(cached!==undefined)return cached;
  const carried=[personasOf(event).size?`as ${[...personasOf(event).values()].join(" ")} persona alias face`:"",(event.mentions||[]).length?`spoken of mentions ${event.mentions.map(id=>entity(id)?.name||"").join(" ")}`:"",event.rewards?.length?`rewards reward ${event.rewards.join(" ")}`:"",String(event.type)==="quest_end"&&event.value?`rewards reward ${event.value}`:"",event.performance?`performance ${event.performance}`:"",event.grade?`grade ${event.grade}`:"",event.authority?`authority ${event.authority}`:""].filter(Boolean).join(" ");
...
// An organization can stand in more than one place at once: a head office, a branch, an outpost.
// Naming the organization as where something happened means one of those places — the branch the
// action names, or, when it names none, the original one: the first the organization opened.
function organizationBranches(id,links){return (links||[]).filter(link=>link.organization===id&&!link.pending).sort((a,b)=>(Number(a.from)||0)-(Number(b.from)||0));}
function branchOf(event,links){
  const branches=organizationBranches(event?.location,links);
  if(!branches.length)return null;
  return (branches.find(link=>link.location===event?.branch)||branches[0]).location;
}
function eventBranchId(event,derived){return entity(event?.location)?.kind==="organization"?branchOf(event,derived?.organizationLocations||[]):null;}
...
// People nest the way places do: a figure that stands for several, and whoever the story has
// picked out of it standing inside. It is never inside anybody itself unless the story says so.
// Taking a figure off the graph asks a question about everyone standing inside it, and only the
// writer knows the answer: the class is dismissed and every face in it is finished with, or the
// class is dismissed and the one girl who matters walks out of it. The action carries the answer.
function goneIds(derived){
  const gone=new Set(derived?.departures?.keys?.()||[]);
  [...(derived?.departures||new Map())].forEach(([id,event])=>{
    if(event?.withInside===false)return;
    const queue=[id],seen=new Set([id]);
    while(queue.length){
      const parent=queue.shift();
      (derived.crowds||[]).forEach(link=>{if(link.parent!==parent||seen.has(link.child))return;seen.add(link.child);gone.add(link.child);queue.push(link.child);});
    }
  });
  return gone;
}
function crowdParentOf(id,derived){const link=(derived?.crowds||[]).find(item=>item.child===id);return link&&!link.pending?link.parent:null;}
function holdsPeople(id,derived){return (derived?.crowds||[]).some(link=>link.parent===id&&!link.pending);}
function buildCrowdView(derived,visibleIds,alsoExpanded=null){
  const people=[...visibleIds].filter(id=>entity(id)?.kind==="character"),
    expanded=alsoExpanded&&alsoExpanded.size?new Set([...expandedCrowds,...alsoExpanded]):expandedCrowds;
  return buildDrillView(people,id=>crowdParentOf(id,derived),expanded);
}
...
function buildSystemView(derived,systemIds,alsoExpanded=null){
  // A subsystem still being taken in stands on its own until the connection settles, the same
  // way a place does.
  const parentOf=new Map(derived.systemParents.filter(link=>!link.pending).map(link=>[link.child,link.parent])),
...
function derive(chapter,eventSubset=null) {
  ...
  data.entities.forEach(item => states.set(item.id,{...item,atLeast:false,displayName:item.name,nameHistory:[],mentioned:actionLimited?null:firstMention(item),appeared:actionLimited?null:firstAppearance(item),aliases:[],level:0,track:DEFAULT_TRACK_ID,realm:"Unrevealed",canonicalRealm:"Unrevealed",status:"unknown",memberships:[]}));
  ...
  const questRun = id => {
    if(!questRuns.has(id))questRuns.set(id,{quest:id,status:"active",progress:0,explicitProgress:false,from:null,to:null,holders:[],updates:[],contributions:[],reward:"",rewards:[],performance:""});
  ...
  (eventSubset||orderedEvents().filter(e=>e.chapter<=chapter)).forEach(event => {
    ...
    if(event.identityIntro===true&&source)source.appeared=source.appeared===null?event.chapter:Math.min(source.appeared,event.chapter);
    ...
    if (event.type === "cultivation" && source) { source.track=trackIdOf(event); source.level = Number(event.level)||1; source.canonicalRealm=cultivationCanonical(event);source.realm=cultivationDisplay(event);source.atLeast=event.atLeast===true; }
    ...
    // A connection does not always happen in one chapter: it can start forming and take many
    // chapters to finish. "begin" puts it on the graph as still forming; the next event of the
    // same kind settles it, and the chapter it started in is kept either way.
    const forming=(previous,record)=>{
      const since=event.action==="forming"?event.chapter:previous?.since;
      return {...record,from:event.chapter,...(since?{since}:{}),...(event.action==="forming"?{pending:true}:{})};
    };
    if (event.type === "membership" && source && states.has(event.target)) {
      const key = event.source+"|"+event.target;
      if (event.action === "leave") memberships.delete(key); else memberships.set(key,forming(memberships.get(key),{character:event.source,organization:event.target,role:event.value||"Member",revealed:event.action==="reveal"}));
    }
    ...
    if(event.type==="system_host"&&source?.kind==="system"&&states.get(event.target)?.kind==="character"){
      const key=event.source+"|"+event.target;
      if(event.action==="remove")systemHosts.delete(key);else systemHosts.set(key,forming(systemHosts.get(key),{system:event.source,host:event.target,role:event.value||"Host"}));
    }
    if(event.type==="system_parent"&&source?.kind==="system"&&states.get(event.target)?.kind==="system"){
      ...
      else systemParents.set(event.source,forming(systemParents.get(event.source),{child:event.source,parent:event.target}));
    }
    ...
    if(String(event.type).startsWith("quest_")&&source?.kind==="quest"){
      if(event.type==="quest_part"){
        ...
        if(event.type==="quest_contribution"&&states.get(event.target)?.kind==="character"){
          ...
          const record={character:event.target,share:event.value||"",rewards:event.rewards||[],performance:event.performance||"",chapter:event.chapter};
        ...
        if(event.type==="quest_end"){
          ...
          if(event.rewards?.length)run.rewards=event.rewards;
    ...
    // Gone from here on. Kept as an event like everything else, so sliding back before it puts
    // them back, and undoing it later brings them back for good.
    if(event.type==="departure"&&source){if(event.action==="remove")departures.delete(event.source);else departures.set(event.source,event);}
    // "Two girls and four guys" is one figure until the story picks somebody out of it. The one
    // picked out is their own identity from that moment, standing inside the figure they came from
    // and keeping every setting of their own.
    if(event.type==="part_of"&&source?.kind==="character"&&states.get(event.target)?.kind==="character"){
      if(event.action==="remove"){if(crowds.get(event.source)?.parent===event.target)crowds.delete(event.source);}
      else crowds.set(event.source,forming(crowds.get(event.source),{child:event.source,parent:event.target}));
    }
    ...
    if(event.type==="identity_parent"&&IDENTITY_KINDS.has(source?.kind)&&IDENTITY_KINDS.has(states.get(event.target)?.kind)){
      if(event.action==="remove"){if(identityParents.get(event.source)?.parent===event.target)identityParents.delete(event.source);}else identityParents.set(event.source,forming(identityParents.get(event.source),{child:event.source,parent:event.target,relation:event.value||"Clone"}));
    }
    ...
    if (event.type === "conversation" && source) {
      ...
      // Talking changes where people stand with each other. Left alone a conversation says
      // nothing about that — two who have only met are neutral — but it can settle it outright.
      if(event.relation)met.forEach((a,index)=>met.slice(index+1).forEach(b=>{
        const key=pairKey(a,b);if(!relations.has(key))relations.set(key,[]);
        relations.get(key).push({...event,type:"relationship",source:a,target:b,value:event.relation});
      }));
    ...
    // Whoever is in an action inside an organization is standing at the branch it resolves to, so
    // the place itself records the visit rather than the organization holding it in the abstract.
    if(states.get(event.location)?.kind==="organization"&&!['mention','membership','organization_location'].includes(event.type)){
      ...
      if(event.type==="movement")locationCharacterIds(event).forEach(character=>locations.set(character,{character,location:branch||event.location,organization:event.location,chapter:event.chapter,order:event.order||0,eventId:event.id,type:event.type,...(event.action==="forming"?{pending:true,since:event.chapter}:{})}));
    }
    if(event.location&&states.get(event.location)?.kind==="location"&&!['mention','residency','location_parent','organization_location'].includes(event.type)){
      ...
      if(event.type==="movement")locationCharacterIds(event).forEach(character=>locations.set(character,{character,location:event.location,chapter:event.chapter,order:event.order||0,eventId:event.id,type:event.type,...(event.action==="forming"?{pending:true,since:event.chapter}:{})}));
    }
  });
...
data.entities.forEach(item=>{const shown=stateName(d,item.id);if(knownIds.has(item.id)&&item.kind!=="quest"){publicOptions.push(`<option value="${escapeHtml(shown)}">${item.kind}</option>`);if(shown!==item.name)publicOptions.push(`<option value="${escapeHtml(item.name)}">Earlier name of ${escapeHtml(shown)}</option>`);known.filter(e=>["alias","display_name"].includes(e.type)&&e.source===item.id).forEach(e=>publicOptions.push(`<option value="${escapeHtml(e.value)}">Known name of ${escapeHtml(shown)}</option>`));}adminOptions.push(`<option value="${escapeHtml(item.name)}">${item.kind}</option>`);data.events.filter(e=>["alias","display_name"].includes(e.type)&&e.source===item.id).forEach(e=>adminOptions.push(`<option value="${escapeHtml(e.value)}">Name of ${escapeHtml(item.name)}</option>`));});
  ...
  $("#location-options").innerHTML=data.entities.filter(item=>["location","organization"].includes(item.kind)).map(item=>`<option value="${escapeHtml(item.name)}">${item.kind==="organization"?"organization":""}</option>`).join("");
  // Any name the story has given out can be worn as a face: an alias, a display name, or another
  // identity that stands on its own.
  const personaOptions=new Set();
  data.events.filter(event=>["alias","display_name"].includes(event.type)&&event.value).forEach(event=>personaOptions.add(String(event.value)));
  data.entities.filter(item=>IDENTITY_KINDS.has(item.kind)).forEach(item=>personaOptions.add(item.name));
  const personaList=$("#persona-options");
  if(personaList)personaList.innerHTML=[...personaOptions].sort((a,b)=>a.localeCompare(b)).map(name=>`<option value="${escapeHtml(name)}"></option>`).join("");
}
...
function renderTimelineMarkers(){
  ...
  if(expandedChapter!==null){const entries=chapterEventEntries(expandedChapter),denominator=entries.length+1;marks.push(`<span class="expanded-boundary-mark previous-boundary" style="left:0" title="Drag here to close and go to the previous chapter">‹</span>`);entries.forEach((entry,index)=>{marks.push(`<button type="button" class="main-timeline-mark expanded-event-mark selectable" style="left:${(index+1)/denominator*100}%;--marker-fill:${EVENT_TYPE_COLORS[entry.event.type]||"#7f8da3"}" title="Event ${index+1} · ${escapeHtml(entry.event.type.replaceAll("_"," "))}" data-event-action="${entry.index}" aria-label="Show event ${index+1} of chapter ${expandedChapter}"></button>`);});marks.push(`<span class="expanded-boundary-mark next-boundary" style="left:100%" title="Drag here to close and go to the next chapter">›</span>`);}
    ...
    // Where the reader is, always, on top of anything else. The slider measures actions or
...
function renderGraph() {
  ...
  const volumeApplied=revealedVolumeActions(),beatEvents=currentBeatEvents(),
    // Every part of a combined moment lands at once, so anything that lights up for "what is
    // happening now" has to look across all of them and not only the last one.
    beatDoes=(type,test)=>beatEvents.some(event=>event.type===type&&test(event)),
    beatOfType=type=>beatEvents.find(event=>event.type===type)||null,
    beatCultivations=beatEvents.filter(event=>event.type==="cultivation"),
    // Actions that have not finished yet: still drawn, still lit, but quieter than the one
    // happening now.
    holdingEvents=heldEvents(),
    holdingIds=new Set(holdingEvents.map(event=>event.id)),
    // The face worn for this beat. The action belongs to the person; what the world saw was a
    // name, and on the graph that name is written over whoever wore it.
    beatPersonas=new Map(beatEvents.flatMap(event=>[...personasOf(event)])),
    previousApplied=volumeActions().slice(0,Math.max(0,currentActionIndex-Math.max(1,beatEvents.length))),currentEvent=currentActionEvent(),appliedNow=appliedEvents(),fullDerived=derive(currentChapter,appliedNow),priorCultivationDerived=beatCultivations.length?derive(currentChapter,appliedNow.filter(event=>!beatCultivations.some(item=>item.id===event.id))):null,volumeDerived=derive(currentChapter,volumeApplied),derived={...volumeDerived,states:fullDerived.states,locationParents:fullDerived.locationParents},departed=goneIds(derived),visibleIds=new Set(volumeApplied.flatMap(event=>[event.source,event.target,event.location,...(event.characters||[]),...(event.mentions||[])].filter(Boolean)).filter(id=>!departed.has(id))),previousVisibleIds=new Set(previousApplied.flatMap(event=>[event.source,event.target,event.location,...(event.characters||[])].filter(Boolean))),chapterChangedIds=new Set(volumeApplied.filter(event=>event.chapter===currentChapter).flatMap(event=>[event.source,event.target,event.location,...(event.characters||[])].filter(Boolean)));
  ...
  const systemAction=beatEvents.find(event=>String(event.type).startsWith("system_")),
    actionSystem=systemAction?[systemAction.source,systemAction.target].find(id=>entity(id)?.kind==="system"):null;
  {
  ...
  const questAction=beatEvents.find(event=>String(event.type).startsWith("quest_")&&event.type!=="quest_part"),
    questBeat=questAction
    ?{quest:questAction.source,name:entity(questAction.source)?.name||"",type:questAction.type,action:questAction.action||"",
      who:new Set([questAction.target,...(questAction.characters||[]),
        ...(derived.quests.find(run=>run.quest===questAction.source)?.holders||[])].filter(Boolean))}
    :null;
  ...
  // Whoever the beat is about has to be on screen for it, even when they normally stand folded
  // inside the figure they were picked out of.
  const beatPeople=new Set([...beatEvents,...holdingEvents].flatMap(event=>[event.source,event.target,...(event.characters||[]),...(event.mentions||[])]).filter(Boolean)),
    openCrowds=new Set();
  beatPeople.forEach(id=>{let cursor=crowdParentOf(id,derived);while(cursor&&!openCrowds.has(cursor)){openCrowds.add(cursor);cursor=crowdParentOf(cursor,derived);}});
  const crowdView=buildCrowdView(derived,visibleIds,openCrowds);lastCrowdView=crowdView;
  [...expandedCrowds].forEach(id=>{if(!crowdView.expanded.has(id))expandedCrowds.delete(id);});
  const renderIds=new Set([...visibleIds].filter(id=>{const kind=entity(id)?.kind;if(kind==="system"||kind==="quest")return false;if(kind==="character")return crowdView.rendered.has(id);return kind!=="location"||locView.rendered.has(id);}));
  ...
  crowdView.rendered.forEach(id=>{if(!crowdView.expanded.has(id))return;(crowdView.children.get(id)||[]).forEach(kid=>addSpring(kid,id,96,.12));});
  ...
  const edgeLocationId=id=>entity(id)?.kind==="character"?(crowdView.anchorOf.get(id)||id):entity(id)?.kind!=="location"?id:(podSet.has(id)?id:(locView.anchorOf.get(id)||id));
  edgeIndex=[];
  ...
  const pairKeys=new Set([...derived.relations.keys(),...derived.awareness.keys(),...derived.meetings.keys()]);
  pairKeys.forEach((key,index)=>{const [aId,bId]=key.split("|"),aPos=positions.get(aId),bPos=positions.get(bId);if(!aPos||!bPos)return;const history=derived.relations.get(key)||[],met=derived.meetings.get(key),aware=derived.awareness.get(key),type=history.length?String(history.at(-1).value).toLowerCase():"neutral";let element;
    ...
    } else if(!history.length&&met){
      // Met, with nothing said yet about how they stand: neutral, which is a standing fact and
      // stays drawn, rather than a line that only appears while somebody is picked out.
      element=svgEl("line",{x1:aPos.x,y1:aPos.y,x2:bPos.x,y2:bPos.y,class:`${edgeClass} met-edge`,stroke:COLORS.neutral,"data-a":aId,"data-b":bId});
  ...
  const forming=link=>link?.pending?" forming-edge":"",
    formingNote=link=>link?.pending?` · still connecting since chapter ${link.since||link.from}`:"";
  derived.memberships.forEach(m=>{noteEdge(m.character,m.organization,m.from,`${m.role}${formingNote(m)}`);return straightEdge(m.character,m.organization,`edge membership-edge${forming(m)}${beatDoes("membership",event=>event.source===m.character&&event.target===m.organization)?" newly-revealed-edge":""}`,m.character,m.organization);});
  ...
  // Opened, a figure and the people picked out of it were joined by nothing at all, so they read
  // as strangers standing near each other. Closed, both ends resolve to the figure and no line is
  // drawn, which is right: there is only one thing there to look at.
  derived.crowds.forEach(link=>{const child=edgeLocationId(link.child),parent=edgeLocationId(link.parent);if(!child||!parent||child===parent)return;noteEdge(child,parent,link.from,`One of them${formingNote(link)}`);straightEdge(child,parent,`edge crowd-edge${forming(link)}${beatDoes("part_of",event=>event.source===link.child&&event.target===link.parent)?" newly-revealed-edge":""}`,child,parent);});
  ...
  derived.conversations.filter(convo=>beatEvents.some(event=>event.id===convo.id)||holdingIds.has(convo.id)||(selectedId&&convo.talkers.includes(selectedId)&&convo.chapter===currentChapter)).forEach(convo=>{
    ...
    const live=beatEvents.some(event=>event.id===convo.id),held=holdingIds.has(convo.id),edgeClass=`edge conversation-edge${live?" newly-revealed-edge":""}${held?" holding-edge":""}`;
    ...
    const hub=svgEl("g",{class:`conversation-hub${live?" conversation-hub-live":""}${held?" conversation-hub-held":""}`,"data-id":hubId});
  ...
  beatEvents.filter(event=>{
    ...
    const group=entity(event.location)?.kind==="organization"?entity(event.location):null,branch=eventBranchId(event,derived),place=edgeLocationId(event.location),
      note=group?`Here for this action — ${group.name}${branch?` · ${entity(branch)?.name||branch}`:""}`:"Here for this action";
    locationCharacterIds(event).forEach(who=>{noteEdge(who,place,event.chapter,note);straightEdge(who,place,"edge location-edge event-place-edge newly-revealed-edge",who,place);});
  });
  ...
  visible.forEach(item=>{const state=derived.states.get(item.id),shownName=state.displayName||item.name,pos=positions.get(item.id),mentionedOnly=item.kind!=="quest"&&state.mentioned!==null&&(state.appeared===null||state.appeared>currentChapter),newlyRevealed=!previousVisibleIds.has(item.id),eventActive=activeIds.has(item.id),eventSpoken=spokenIds.has(item.id),chapterChanged=chapterChangedIds.has(item.id),cultivationReveal=beatDoes("cultivation",event=>event.source===item.id),priorCultivationState=cultivationReveal?priorCultivationDerived?.states.get(item.id):null,priorCultivationLevel=cultivationReveal?(priorCultivationState?.level||0):(state.level||0),openedLocation=((item.kind==="location"||item.kind==="organization")&&locView.expanded.has(item.id))||(item.kind==="system"&&sysView.expanded.has(item.id)),emerging=(item.kind==="location"||item.kind==="system"||item.kind==="organization")&&emergingLocations.has(item.id),retracting=retractingIds.has(item.id)||item.id===collapsingLocationId,group=svgEl("g",{class:`node ${item.kind}${item.kind==="system"&&isSpecialGrade(state.grade)?` system-${String(state.grade).trim().toLowerCase()}`:""}${mentionedOnly?" mentioned-only":""}${newlyRevealed?" newly-revealed-node":""}${chapterChanged?" chapter-changed-node":""}${eventActive?" event-active-node":""}${eventSpoken?" event-spoken-node":""}${cultivationReveal?" cultivation-reveal":""}${openedLocation?" location-opened":""}${emerging?" location-emerging":""}${retracting?" location-retracting":""}`,"data-id":item.id,role:"button",tabindex:0,"aria-label":eventSpoken?`${shownName}, spoken of here`:mentionedOnly?`${shownName}, mentioned but not appeared`:shownName,transform:`translate(${pos.x},${pos.y})`});let labelY=item.kind==="character"?5:58,locationShell=null,rankLabel=null,questNotice=null,personaNotice=null;
    if(item.kind==="organization"){const holds=(locView.children.get(item.id)||[]).length;
      ...
      group.appendChild(locationShell);
    }else{const appeared=state.appeared!==null&&state.appeared<=currentChapter,r=appeared?radius(state):20,inside=(crowdView.children.get(item.id)||[]).length,opened=crowdView.expanded.has(item.id);
      group.appendChild(svgEl("circle",{cx:0,cy:0,r:Math.max(44,r+22),class:"node-hit-target"}));
      // A figure that stands for several people, opened, is a ring with them around it — the same
      // reading a place gives when it is opened into what it holds.
      if(inside&&opened){group.append(svgEl("circle",{cx:0,cy:0,r:17,class:"crowd-open-halo"}),svgEl("circle",{cx:0,cy:0,r:6.5,class:"crowd-open-dot"}));labelY=-26;group.setAttribute("aria-label",`${shownName}, opened — ${inside} picked out of it, activate to close`);}
      if(beatPersonas.has(item.id))personaNotice={y:r+27,text:`really ${shownName}`};
    ...
    const labelGroup=svgEl("g",{class:`${group.getAttribute("class")} node-labels`,"data-id":item.id}),label=svgEl("text",{x:0,y:labelY,class:`node-label${item.kind==="location"?" location-region-label":""}${personaNotice?" worn-name":""}`});label.textContent=personaNotice?beatPersonas.get(item.id):shownName;labelGroup.appendChild(label);
    ...
    if(personaNotice){const worn=svgEl("text",{x:0,y:personaNotice.y,class:"persona-notice"});worn.textContent=personaNotice.text;labelGroup.appendChild(worn);}
    ...
    const selectNode=()=>{cancelChapterSequence();if(item.kind==="character"&&(crowdView.children.get(item.id)||[]).length){activateCrowd(item.id);return;}if(item.kind==="location"||(locView.children.get(item.id)||[]).length){activateLocation(item.id);return;}if(item.kind==="system"){activateSystem(item.id);return;}locationPovId=null;selectedId=selectedId===item.id?null:item.id;setMobilePanel("info");renderAll();};
...
// The same cycle a place gives: first click picks the figure out, second opens it into the people
// standing inside, and the dot itself folds them back in.
function activateCrowd(id){
  const view=lastCrowdView;
  if(!view||!view.rendered.has(id)){locationPovId=null;selectedId=selectedId===id?null:id;setMobilePanel("info");renderAll();return;}
  if(view.expanded.has(id)){expandedCrowds.delete(id);renderedSubtree(id,view).forEach(child=>{expandedCrowds.delete(child);physics.pos.delete(child);physics.vel.delete(child);physics.bounds.delete(child);});if(selectedId&&renderedSubtree(id,view).includes(selectedId))selectedId=null;renderAll();return;}
  if(selectedId===id&&(view.children.get(id)||[]).length){
    const kids=view.children.get(id)||[],origin=physics.pos.get(id);
    expandedCrowds.add(id);selectedId=null;locationPovId=null;
    ...
    setMobilePanel("info");renderAll();toast(`${entity(id)?.name||id} opened — ${kids.length} picked out of it`);return;
  }
  locationPovId=null;selectedId=selectedId===id?null:id;setMobilePanel("info");renderAll();
}
...
// A completion's message says it completed; what it paid is on the action itself, and that is the
// part worth reading. It is written out wherever the action is, not only in the quest tab.
function actionPaidHtml(event){
  const rewards=event.rewards||[],performance=event.performance||"";
...
function renderEvents(){const list=$("#events-list"),event=currentActionEvent(),card=list.closest(".events-card");
  ...
  // Something begun in an earlier chapter and still running belongs at the top of what is
  // happening now, or the reader loses it the moment the chapter turns.
  const carried=heldEvents().filter(item=>item.chapter!==event.chapter);
  list.innerHTML=`${carried.length?`<li class="event-chapter-heading still-going"><span>Still going</span><small>from earlier</small></li>${carried.map(item=>eventPanelRow(item,volumeActions().findIndex(action=>action.id===item.id)+1,{related:true})).join("")}<li class="event-chapter-heading"><span>Chapter ${event.chapter}</span><small>${chapterRows.length} action${chapterRows.length===1?"":"s"}</small></li>`:""}${chapterRows.map(entry=>beatPanelRow(entry,{current:entry.index===currentActionIndex,upcoming:entry.index>currentActionIndex})).join("")}`;
  bindEventPanelRows();bindMomentParts();
...
function questCardHtml(run,derived,children,depth,changedId=null,lifted=false){
  ...
  const open=expandedQuests.has(run.quest),partsOpen=!foldedQuestChains.has(run.quest),
    ...
    rewardList=run.rewards?.length?run.rewards:item.rewards?.length?item.rewards:[],
    rewardText=rewardList.length?rewardList.join(" · "):run.reward||(settled?"":"Told on completion"),
    rank=item.rating||"",
    facts=[
      ...
      rewardList.length>1?{label:"Rewards",value:`<ul class="quest-reward-list">${rewardList.map(reward=>`<li>${richInline(reward)}</li>`).join("")}</ul>`,html:true}:rewardText?{label:rewardList.length?"Reward":"Rewards",value:rewardText}:null,
  ...
  // A quest that moved this beat is worth noticing, so the card says so rather than quietly
  // reading a different number than it did a moment ago.
  const stirred=changedId===run.quest;
  return `<article class="quest-card tone-${questProgressTone(run)} depth-${Math.min(2,depth)}${open?" quest-open":""}${stirred?" quest-stirred":""}${lifted?" quest-lifted":""}" data-quest="${escapeHtml(run.quest)}" style="--quest-progress:${run.progress}%">
    ...
    ${open?`<div class="quest-detail">${item.description?`<p class="quest-description">${richText(item.description)}</p>`:""}
      ${facts.length?`<dl class="quest-facts">${facts.map(fact=>`<div><dt>${escapeHtml(fact.label)}</dt><dd>${fact.html?fact.value:richInline(fact.value)}</dd></div>`).join("")}</dl>`:""}
      ${run.contributions.length?`<dl class="quest-facts quest-shares">${run.contributions.map(share=>`<div><dt>${escapeHtml(stateName(derived,share.character)||share.character)}</dt><dd>${escapeHtml([share.share,share.performance?`performance ${share.performance}`:"",share.rewards?.length?`rewarded ${share.rewards.join(" · ")}`:""].filter(Boolean).join(" · ")||"contributed")}</dd></div>`).join("")}</dl>`:""}
    ...
    ${!open&&(rewardList.length||run.performance)?`<div class="quest-paid">${rewardList.length?`<span class="quest-paid-label">Rewards</span>${rewardList.map(reward=>`<b class="quest-reward">${richInline(reward)}</b>`).join("")}`:""}${run.performance?`<span class="quest-performance">performance ${escapeHtml(run.performance)}</span>`:""}</div>`:""}
    ${kids.length&&partsOpen?`<div class="quest-children">${kids.map(child=>questCardHtml(child,derived,children,depth+1,changedId)).join("")}</div>`:""}
  </article>`;
}
function renderQuests(){
  ...
  // A part is counted with the quest it belongs to unless the story treats it as one of its own,
  // which is why a chain of three could read as "1 active" while three cards were on screen.
  const activeTotal=derived.quests.filter(run=>run.status==="active"&&(!parentOf.has(run.quest)||entity(run.quest)?.countsAlone===true)).length;
  count.textContent=activeTotal?`${activeTotal} active`:derived.quests.length?"none active":"";
  ...
  // Quests from different systems are different business and are never mixed together: each
  // issuer keeps its own section, in the order its first quest appeared.
  const sections=new Map(),issuerOf=run=>entity(run.quest)?.issuer||"";
  active.forEach(run=>{const key=issuerOf(run);if(!sections.has(key))sections.set(key,[]);sections.get(key).push(run);});
  const sectionKeys=[...sections.keys()],
    settledShown=questSettledOpen?settled.slice(0,QUEST_PAGE+questPageSettled):[],
    settledRest=settled.length-settledShown.length;

  // Whatever just changed is lifted to the top for its own beat, then takes its place in its
  // issuer's section like everything else.
  const beat=currentActionEvent(),
    justChanged=beat&&String(beat.type).startsWith("quest_")&&beat.type!=="quest_part"?runs.get(beat.source):null;

  const sectionHtml=key=>{
    const list=sections.get(key)||[],issuer=key?entity(key):null,
      shown=list.slice(0,QUEST_PAGE+(questPageActive||0)),rest=list.length-shown.length;
    return `<section class="quest-section">
      <h4 class="quest-section-head">${issuer?`<span class="quest-section-issuer">${escapeHtml(stateName(derived,key)||issuer.name)}</span>`:'<span class="quest-section-issuer none">Not issued by anyone</span>'}<span class="quest-section-count">${list.length}</span></h4>
      <div class="quest-group">${shown.map(run=>questCardHtml(run,derived,children,0,justChanged?.quest)).join("")}</div>
      ${rest>0?`<div class="quest-page-more"><button type="button" data-quest-more="${escapeHtml(key)}">Show ${Math.min(rest,QUEST_PAGE)} more · ${rest} left</button></div>`:""}
    </section>`;
  };

  box.innerHTML=`${justChanged?`<div class="quest-latest-change"><span class="quest-latest-head">Just now</span>${questCardHtml(justChanged,derived,children,0,justChanged.quest,true)}</div>`:""}
    ${sectionKeys.length?sectionKeys.map(sectionHtml).join(""):'<p class="quests-empty">Nothing is running right now.</p>'}
  ...
  box.querySelectorAll("[data-quest-more]").forEach(button=>button.addEventListener("click",()=>{questPageActive+=QUEST_PAGE;renderQuests();}));
...
function actionSubjectLine(event){
  const name=id=>id?(entity(id)?.name||id):"",
    worn=personasOf(event),
    parts=[name(event.source)+(worn.get(event.source)?` as ${worn.get(event.source)}`:""),name(event.target)+(worn.get(event.target)?` as ${worn.get(event.target)}`:""),event.location?`at ${name(event.location)}${event.branch?` · ${name(event.branch)}`:""}`:"",(event.mentions||[]).length?`speaks of ${event.mentions.map(name).join(", ")}`:""].filter(Boolean);
...
function actionEffectProblem(event){
  ...
  if(type==="departure")return needs(["character","organization","location","system"].includes(kind(event.source)),"this needs somebody or somewhere to be gone");
  if(type==="part_of")return needs(kind(event.source)==="character"&&kind(event.target)==="character","this needs a person and the figure they were picked out of");
...
// The branch question only exists once the where is an organization, and the only answers to it
// are that organization's own places: nothing else can be a branch of it.
function updateBranchField(){
  ...
  const branches=organizationBranches(where.id,derive(validChapter(form.elements.chapter.value)||currentChapter).organizationLocations);
  $("#branch-options").innerHTML=branches.map(link=>`<option value="${escapeHtml(entity(link.location)?.name||link.location)}">${escapeHtml(link.role)}</option>`).join("");
  $("#event-branch-note").textContent=branches.length
    ?`Only when one branch takes part rather than the whole of ${where.name}. Left empty it is ${entity(branches[0].location)?.name||branches[0].location}, the first place it opened.`
...
function updateEventHelp(){
  ...
  if(charactersLabel)charactersLabel.textContent=type==="conversation"?"Everyone taking part":type==="note"?"Everyone else in it (optional)":type==="movement"?"Who else travels with them (optional)":"Who it is issued to";
  if(charactersNote)charactersNote.textContent=type==="conversation"
    ...
    :type==="note"?"Separate names with commas. Plenty of things happen to more than one person at once — one action covers all of them, rather than a copy of the same line each."
    :type==="movement"?"Separate names with commas. People who arrive together arrive in one action; each of them ends up at the destination."
    :"Separate names with commas. A quest issued to more than one person is one action, not several.";
  $("#event-source-label").textContent=type==="part_of"?"Who is picked out":type==="departure"?"Who or what is gone":String(type).startsWith("quest_")?"Quest":type==="conversation"?"Who speaks first":type==="movement"?"Character travelling":isOrgLocation?"Organization":isResidence?"Resident character":isHierarchy?"Child location":isIdentityHierarchy?"Clone / avatar / child identity":type==="note"?"Main character, organization, or location":"Identity / source";
  $("#event-target-label").textContent=type==="part_of"?"The figure they are one of":type==="quest_issue"?"Issued to (optional)":type==="quest_part"?"Larger quest this is part of":type==="quest_contribution"?"Who contributed":type==="membership"?"Organization":isIdentityHierarchy?"Original / parent identity":type==="note"?"Related identity (optional)":"Second character";
  $("#event-value-label").textContent=type==="departure"?"Why they are gone (optional)":type==="system_end"?"Why it was destroyed (optional)":type==="quest_end"?"What the reward turned out to be (optional)":type==="quest_contribution"?"Share of the work (optional)":type==="display_name"?"New public display name":type==="identity_parent"?"Identity relationship":type==="alias"?"New alias":type==="cultivation"?"Equivalent path name (optional)":type==="status"?"New status":type==="gender"?"Gender":type==="age"?"Age":type==="relationship"?"Relationship (friendly, neutral, or hostile)":isOrgLocation?"Organization location role":isResidence?"Residence type":"Role or title (optional)";
  form.elements.target.placeholder=type==="part_of"?"E.g. Two girls and four guys":type==="quest_issue"?"Character who receives it":type==="quest_part"?"The larger quest this sits inside":type==="quest_contribution"?"Character whose share this is":type==="membership"?"Type an organization name":isIdentityHierarchy?"Original character, parent clone, or the system they merged into":type==="note"?"Optional":"Type a character name or alias";
  form.elements.value.placeholder=type==="departure"?"Killed at the gate / never seen again / demolished":type==="system_end"?"Burned out in the Jotun campaign":type==="quest_end"?"Realm Seed, 200 contribution points":type==="quest_contribution"?"Two thirds of the work":type==="display_name"?"Name readers see from this action":type==="identity_parent"?"Clone / Avatar / Incarnation / Merged into":type==="alias"?"E.g. The Innkeeper":type==="cultivation"?"E.g. Earthen Deity":type==="status"?"Unknown / Alive / Dead / Missing":type==="gender"?"E.g. Male, Female, Unknown, Nonbinary":type==="age"?"E.g. 17, Early twenties, Over 900 years":type==="relationship"?"friendly / neutral / hostile":isOrgLocation?"Headquarters / Branch / Base / Outpost":isResidence?"Home / Permanent resident / Camp":"E.g. Founder or member";
  if(isOrgLocation)form.elements.value.setAttribute("list","location-role-options");else if(isResidence)form.elements.value.setAttribute("list","residence-role-options");else if(isIdentityHierarchy)form.elements.value.setAttribute("list","identity-relation-options");else form.elements.value.removeAttribute("list");
  const action=form.elements.action,keepAction=action.value;
  if(type==="movement"){action.innerHTML='<option value="add">Arrives there</option><option value="forming">On the way — has not arrived yet</option>';$("#event-action-label").textContent="Travel";}
  else if(type==="system_host"){action.innerHTML='<option value="add">Bonds to this host</option><option value="forming">Bond is forming — not sealed yet</option><option value="remove">Bond ends</option>';$("#event-action-label").textContent="System host change";}
  else if(type==="system_parent"){action.innerHTML='<option value="add">Becomes a subsystem</option><option value="forming">Being taken in — not a subsystem yet</option><option value="remove">No longer a subsystem</option>';$("#event-action-label").textContent="Subsystem change";}
  else if(type==="system_location"){action.innerHTML='<option value="open">Starts operating here</option><option value="forming">Opening here — not running yet</option><option value="close">Withdraws from here</option>';$("#event-action-label").textContent="System location change";}
  ...
  else if(type==="departure"){action.innerHTML='<option value="add">Gone from here on</option><option value="remove">Back on the graph</option>';$("#event-action-label").textContent="Taken off the graph";}
  else if(type==="part_of"){action.innerHTML='<option value="add">One of it</option><option value="forming">Being counted among them — not yet</option><option value="remove">No longer one of it</option>';$("#event-action-label").textContent="Standing inside";}
  ...
  else if(isOrgLocation){action.innerHTML='<option value="open">Establishes / opens here</option><option value="forming">Being established — not open yet</option><option value="close">Closes / leaves this location</option>';$("#event-action-label").textContent="Organization location change";}else if(isResidence){action.innerHTML='<option value="begin">Residence begins</option><option value="forming">Moving in — not settled yet</option><option value="end">Residence ends</option>';$("#event-action-label").textContent="Residence change";}else if(isHierarchy){action.innerHTML='<option value="add">Place inside parent</option><option value="forming">Connecting to it — not joined yet</option><option value="remove">Remove from parent</option>';$("#event-action-label").textContent="Location hierarchy change";}else if(isIdentityHierarchy){action.innerHTML='<option value="add">Connect to parent identity</option><option value="forming">Taking shape — not separate yet</option><option value="remove">Separate from parent identity</option>';$("#event-action-label").textContent="Identity hierarchy change";}else{action.innerHTML='<option value="reveal">Existing membership is revealed</option><option value="join">Joins in this chapter</option><option value="forming">Joining — not a member yet</option><option value="leave">Leaves / membership ends</option>';$("#event-action-label").textContent="Organization membership knowledge";}
...
// "A and B and C" is not how anybody writes a cast list.
function nameList(names){const all=[...new Set(names.filter(Boolean))];return all.length<3?all.join(" and "):`${all.slice(0,-1).join(", ")} and ${all.at(-1)}`;}
function generatedEventDescription(type,source,target,location,value,level,action,initial=false,trackId=DEFAULT_TRACK_ID,participants=[],rankAuthority="",rankGrade="",progress="",atLeast=false){
...
function buildEventRecord(formElement,id="ev-"+crypto.randomUUID()){
  ...
  if(type==="departure"&&!["character","organization","location","system"].includes(source.kind)){toast("Choose the character, place, organization, or system that is gone");return null;}
  if(type==="part_of"&&(source.kind!=="character"||target?.kind!=="character")){toast("Somebody is picked out of a figure that also stands on the graph as a character");return null;}
  if(type==="part_of"&&source.id===target.id){toast("Nobody is one of themselves");return null;}
  if(type==="part_of"&&action!=="remove"){let cursor=crowdParentOf(target.id,derive(chapter)),guard=new Set([target.id]);while(cursor&&!guard.has(cursor)){if(cursor===source.id){toast("That would put each of them inside the other");return null;}guard.add(cursor);cursor=crowdParentOf(cursor,derive(chapter));}}
  ...
  const previous=data.events.find(event=>event.id===id),enteredDescription=String(form.get("description")||"").trim(),previousGenerated=previous?generatedEventDescription(previous.type,entity(previous.source),entity(previous.target),entity(previous.location),previous.value,previous.level,previous.action,previous.initial===true,trackIdOf(previous),(previous.characters||[]).map(id=>entity(id)?.name).filter(Boolean),previous.authority??"",previous.grade||"",previous.progress??"",previous.atLeast===true):"",useGenerated=!enteredDescription||enteredDescription===previousGenerated||isAutomaticCultivationDescription(enteredDescription,previous),description=useGenerated?generatedEventDescription(type,source,target,location,value,level,action,previous?.initial===true,String(form.get("track")||progressionTracks()[0].id),String(form.get("characters")||"").split(",").map(part=>part.trim()).filter(Boolean).map(part=>resolveEntity(part)?.name).filter(Boolean),String(form.get("authority")||"").trim(),String(form.get("grade")||"").trim(),String(form.get("progress")||"").trim(),Boolean(form.get("atLeast"))):enteredDescription,order=previous?.chapter===chapter&&previous?.order?previous.order:nextEventOrder(chapter,eventDrafts),record={id,chapter,order,type,source:source.id,description,...(sourceUrl?{sourceUrl:safeExternalUrl(sourceUrl)}:{})};
  if(type==="cultivation"&&form.get("atLeast"))record.atLeast=true;
  if(type==="departure"&&!form.get("withInside"))record.withInside=false;
  if(location)record.location=location.id;
  // Which branch of it: only meaningful when the where is an organization, and only one of the
  // places that organization is actually based at. Left empty, the original branch stands in.
  const branchText=String(form.get("branch")||"").trim(),branch=branchText?resolveEntity(branchText):null;
  if(branchText&&location?.kind!=="organization"){toast("A branch belongs to an organization. Name the organization as where this happened, or clear the branch.");return null;}
  if(branchText&&!branch){toast("Choose an existing place as the branch");return null;}
  if(branch&&!organizationBranches(location.id,derive(chapter).organizationLocations).some(link=>link.location===branch.id)){toast(`${location.name} has no branch at ${branch.name} by chapter ${chapter}. Record that first with an organization location action.`);return null;}
  if(branch)record.branch=branch.id;
  // Who did it and which face they wore are different questions. The graph keeps them apart:
  // the action belongs to the person, and says which name the world saw doing it.
  const heldFor=String(form.get("holds")||"").trim();
  if(heldFor&&!/^\d{1,3}$/.test(heldFor)){toast("How long it lasts is a whole number of actions, or empty");return null;}
  if(Number(heldFor)>0)record.holds=Number(heldFor);
  const conversationRelation=String(form.get("relation")||"").trim();
  if(type==="conversation"&&conversationRelation)record.relation=conversationRelation;
  const worn={};
  String(form.get("persona")||"").split(",").map(part=>part.trim()).filter(Boolean).forEach(part=>{
    const split=part.match(/^(.*?)\s+(?:as|:)\s+(.*)$/i),
      who=split?resolveEntity(split[1]):source,face=(split?split[2]:part).trim();
    if(!who||!face)return;
    if(face.toLowerCase()===String(stateName(currentDerived(),who.id)||who.name).toLowerCase())return;
    worn[who.id]=face;
  });
  if(Object.keys(worn).length)record.personas=worn;
  // Being spoken of is not being there. What an action refers to is kept apart from who was in it
  // and where it happened, and is drawn as a reference rather than a presence.
  const spokenOf=String(form.get("mentions")||"").split(",").map(part=>part.trim()).filter(Boolean).map(resolveEntity);
  if(spokenOf.some(item=>!item)){toast("One of the names spoken of does not match an identity");return null;}
  const here=new Set([source.id,target?.id,location?.id,...(record.characters||[])].filter(Boolean)),
    mentions=[...new Set(spokenOf.map(item=>item.id))].filter(id=>!here.has(id));
  ...
  if(["awareness","meeting","relationship","membership","identity_parent","system_host","system_parent","system_merge","part_of","quest_part","quest_contribution","quest_issue"].includes(type)||(type==="note"&&target))record.target=target?.id;
  ...
  // Plenty of things happen to more than one person at once, and travel is usually done together.
  // One action carries the whole cast rather than forcing a copy of the same line for each of them.
  if(["note","movement"].includes(type)){
    const named=String(form.get("characters")||"").split(",").map(part=>part.trim()).filter(Boolean).map(resolveEntity);
    if(named.some(item=>!item)){toast("One of those names does not match an identity");return null;}
    if(type==="movement"){const stuck=named.find(item=>item.kind!=="character");if(stuck){toast(`${stuck.name} is a ${stuck.kind} — only a character can travel`);return null;}}
    const everyone=[...new Set([source.id,...named.map(item=>item.id)])];
  ...
  if(String(type).startsWith("quest_")){
    ...
    if(["quest_end","quest_contribution"].includes(type)){
      // A reward is whatever the story hands over — an item, a quantity, a rank, a title — and a
      // quest can pay several at once. Only the mission itself carries a rating.
      const rewards=listFromText(form.get("rewards"),true),performance=String(form.get("performance")||"").trim();
      ...
      if(rewards.length)record.rewards=rewards;
  ...
  if(["membership","organization_location","residency","location_parent","identity_parent","system_host","system_parent","system_location","system_merge","system_end","departure","part_of","quest_issue","quest_end","quest_part"].includes(type))record.action=action;
  return record;
}
...
$("#entity-form").addEventListener("submit",event=>{
  ...
  if(kind==="character"){
    ...
    const cultivationChapter=appeared||mentioned;if(initialLevel){const cultivationRecord=initialCultivationEvent||{id:"ev-"+crypto.randomUUID(),type:"cultivation",track:String(form.get("initialTrack")||progressionTracks()[0].id),source:id},presenceEvent=data.events.filter(storyEvent=>storyEvent.source===id&&storyEvent.creationManaged===true&&["mention","appearance"].includes(storyEvent.type)&&storyEvent.chapter===cultivationChapter).sort((a,b)=>(a.order||0)-(b.order||0)).at(-1),sameChapter=cultivationRecord.chapter===cultivationChapter,cultivationOrder=sameChapter&&cultivationRecord.order?cultivationRecord.order:presenceEvent?(Number(presenceEvent.order)||1)+.01:nextEventOrder(cultivationChapter),shownTier=initialCultivationAlias?`${initialCultivationAlias}, equivalent to ${CULTIVATION_LEVELS[initialLevel-1]}`:CULTIVATION_LEVELS[initialLevel-1],generated=form.get("initialAtLeast")?`${name} is at least ${shownTier}.`:`${name}'s cultivation is revealed as ${shownTier}.`;Object.assign(cultivationRecord,{chapter:cultivationChapter,order:cultivationOrder,creationManaged:true,initial:true,level:initialLevel,atLeast:form.get("initialAtLeast")?true:undefined,value:initialCultivationAlias||CULTIVATION_LEVELS[initialLevel-1],description:creationDescription(initialCultivationEvent,generated),...sourceFields});if(!creationSourceUrl)delete cultivationRecord.sourceUrl;if(!initialCultivationEvent)data.events.push(cultivationRecord);}else if(initialCultivationEvent)data.events=data.events.filter(storyEvent=>storyEvent.id!==initialCultivationEvent.id);
  }else{const previousMention=managedBefore.find(storyEvent=>storyEvent.type==="mention");data.events=data.events.filter(storyEvent=>!(storyEvent.source===id&&isCreationManagedEvent(storyEvent)&&["mention","appearance"].includes(storyEvent.type)));
    // A place or a group can be spoken of long before the story ever shows it, exactly as a person
    // can. Until it is introduced it stands on the graph as spoken of and nothing more.
    if(mentioned){const sameMentionChapter=previousMention?.chapter===mentioned,mentionText=`${name} is mentioned for the first time.`;data.events.push({id:previousMention?.id||"ev-"+crypto.randomUUID(),chapter:mentioned,order:sameMentionChapter&&previousMention?.order?previousMention.order:nextEventOrder(mentioned),type:"mention",source:id,creationManaged:true,description:creationDescription(previousMention,mentionText),...sourceFields});}
    const introEvent=managedBefore.find(storyEvent=>storyEvent.type==="note"),generated=`${name} is introduced.`;if(!appeared){if(introEvent)data.events=data.events.filter(storyEvent=>storyEvent.id!==introEvent.id);}else if(introEvent){const sameChapter=introEvent.chapter===appeared;introEvent.chapter=appeared;introEvent.order=sameChapter&&introEvent.order?introEvent.order:nextEventOrder(appeared);introEvent.creationManaged=true;introEvent.identityIntro=true;introEvent.description=creationDescription(introEvent,generated);if(creationSourceUrl)introEvent.sourceUrl=safeExternalUrl(creationSourceUrl);else delete introEvent.sourceUrl;if(kind==="location")introEvent.location=id;else delete introEvent.location;}else data.events.push({id:"ev-"+crypto.randomUUID(),chapter:appeared,order:nextEventOrder(appeared),type:"note",source:id,creationManaged:true,identityIntro:true,...sourceFields,...(kind==="location"?{location:id}:{}),description:generated});}
...
$("#event-form").elements.location.addEventListener("input",updateBranchField);
$("#event-form").elements.chapter.addEventListener("input",updateBranchField);
...
function syncSliderModeToggle(){const button=$("#toggle-slider-mode");if(!button)return;
  button.setAttribute("aria-pressed",String(sliderStepsEvents));button.classList.toggle("active",sliderStepsEvents);
  $("#slider-mode-label").textContent=sliderStepsEvents?"By action":"By chapter";}
sliderStepsEvents=localStorage.getItem(SLIDER_MODE_KEY)==="events";
$("#toggle-slider-mode")?.addEventListener("click",()=>{
  // Keep the reader where they are: the position is an action index either way.
  sliderStepsEvents=!sliderStepsEvents;expandedChapter=null;cancelChapterSequence();
