...
2 | https://www.webnovel.com/book/.../chapter-title_789012"></textarea></label>
          ...
          <div class="admin-grid">
            <form id="entity-form" class="admin-card"><div class="admin-card-heading"><h2 id="entity-form-title">Create character, organization, or location</h2><div class="manage-row"><input id="manage-entity" list="admin-entity-options" placeholder="Load an existing identity"><button class="button ghost" id="load-entity" type="button">Load</button></div></div><p id="creation-edit-note" class="event-edit-mode-note" hidden><strong>Editing a creation-generated row.</strong> These facts were made by this identity form, so saving here updates the original linked row instead of creating a chapter event.</p><input name="editingId" type="hidden"><input name="editingCreationEventId" type="hidden"><div class="form-grid">
              ...
              <label class="field" id="entity-authority-field" hidden><span>Authority</span><input name="authority" type="number" step="1" placeholder="7" /><small class="field-note">A plain number. Higher means more authority.</small></label>
              <label class="field" id="entity-grade-field" hidden><span>Grade</span><input name="grade" list="system-grade-options" maxlength="7" placeholder="S" /><datalist id="system-grade-options"><option value="Divine"></option><option value="Death"></option><option value="Life"></option></datalist><small class="field-note">Up to three characters — S, A+, SS — or one of the special grades: Divine, Death, Life.</small></label>
...
function gradeIsValid(grade){const text=String(grade||"").trim();return !text||isSpecialGrade(text)||text.length<=3;}
...
function revealedVolumeActions(){return [...volumeActions().slice(0,currentActionIndex),...ghostActions()];}
...
function renderGraph() {
  ...
  if(selectedId){
    ...
    while(queue.length){const id=queue.shift();if(!id||revealedSystems.has(id)||entity(id)?.kind!=="system"||retired.has(id))continue;revealedSystems.add(id);derived.systemParents.filter(link=>link.parent===id).forEach(link=>queue.push(link.child));}
  ...
  visible.forEach(item=>{const state=derived.states.get(item.id),shownName=state.displayName||item.name,pos=positions.get(item.id),mentionedOnly=item.kind==="character"&&state.mentioned!==null&&(state.appeared===null||state.appeared>currentChapter),newlyRevealed=!previousVisibleIds.has(item.id),eventActive=activeIds.has(item.id),chapterChanged=chapterChangedIds.has(item.id),cultivationReveal=currentEvent?.type==="cultivation"&&currentEvent.source===item.id,priorCultivationState=cultivationReveal?priorCultivationDerived?.states.get(item.id):null,priorCultivationLevel=cultivationReveal?(priorCultivationState?.level||0):(state.level||0),openedLocation=(item.kind==="location"&&locView.expanded.has(item.id))||(item.kind==="system"&&sysView.expanded.has(item.id)),emerging=(item.kind==="location"||item.kind==="system")&&emergingLocations.has(item.id),retracting=retractingIds.has(item.id)||item.id===collapsingLocationId,group=svgEl("g",{class:`node ${item.kind}${item.kind==="system"&&isSpecialGrade(item.grade)?` system-${String(item.grade).trim().toLowerCase()}`:""}${mentionedOnly?" mentioned-only":""}${newlyRevealed?" newly-revealed-node":""}${chapterChanged?" chapter-changed-node":""}${eventActive?" event-active-node":""}${cultivationReveal?" cultivation-reveal":""}${openedLocation?" location-opened":""}${emerging?" location-emerging":""}${retracting?" location-retracting":""}`,"data-id":item.id,role:"button",tabindex:0,"aria-label":mentionedOnly?`${shownName}, mentioned but not appeared`:shownName,transform:`translate(${pos.x},${pos.y})`});let labelY=item.kind==="character"?5:58,locationShell=null;
    if(item.kind==="organization"){const points=Array.from({length:6},(_,i)=>{const angle=Math.PI/3*i-Math.PI/6;return `${39*Math.cos(angle)},${39*Math.sin(angle)}`}).join(" ");group.append(svgEl("circle",{cx:0,cy:0,r:46,class:"node-hit-target"}),svgEl("polygon",{points,class:"org-shape"}));
      ...
      if(opened){labelY=-28;locationShell.append(svgEl("circle",{cx:0,cy:0,r:18,class:"system-open-halo"}),svgEl("circle",{cx:0,cy:0,r:6.5,class:"system-open-dot"}));
        ...
        if(item.grade){const grade=svgEl("text",{x:0,y:isSpecialGrade(item.grade)?3:5,class:`system-grade${isSpecialGrade(item.grade)?" system-grade-special":""}`});grade.textContent=isSpecialGrade(item.grade)?String(item.grade).trim().toUpperCase().slice(0,4):String(item.grade).trim().toUpperCase();locationShell.appendChild(grade);}
    ...
    if(item.kind==="location"||item.kind==="system"){const tier=svgEl("text",{x:0,y:labelY-13,class:`location-tier-label${item.kind==="system"?" system-tier-label":""}`});tier.textContent=item.kind==="system"?(Number.isFinite(item.authority)?`SYSTEM · AUTHORITY ${item.authority}`:"SYSTEM"):String(item.locationType||"Place").toUpperCase();labelGroup.appendChild(tier);}
...
$("#entity-form").addEventListener("submit",event=>{
  ...
  if(kind==="system"&&!gradeIsValid(form.get("grade"))){toast("A grade is at most three characters, or one of Divine, Death, Life");return null;}
