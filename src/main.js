...
function actionHaystack(event){
  ...
  const carried=[event.persona?`as ${event.persona} persona alias face`:"",event.rewards?.length?`rewards reward ${event.rewards.join(" ")}`:"",String(event.type)==="quest_end"&&event.value?`rewards reward ${event.value}`:"",event.performance?`performance ${event.performance}`:"",event.grade?`grade ${event.grade}`:"",event.authority?`authority ${event.authority}`:""].filter(Boolean).join(" ");
...
function actionSubjectLine(event){
  const name=id=>id?(entity(id)?.name||id):"",
    parts=[name(event.source)+(event.persona?` as ${event.persona}`:""),name(event.target),event.location?`at ${name(event.location)}`:""].filter(Boolean);
