...
2 | https://www.webnovel.com/book/.../chapter-title_789012"></textarea></label>
          ...
          <div class="admin-grid">
            <form id="entity-form" class="admin-card"><div class="admin-card-heading"><h2 id="entity-form-title">Create character, organization, or location</h2><div class="manage-row"><input id="manage-entity" list="admin-entity-options" placeholder="Load an existing identity"><button class="button ghost" id="load-entity" type="button">Load</button></div></div><p id="creation-edit-note" class="event-edit-mode-note" hidden><strong>Editing a creation-generated row.</strong> These facts were made by this identity form, so saving here updates the original linked row instead of creating a chapter event.</p><input name="editingId" type="hidden"><input name="editingCreationEventId" type="hidden"><div class="form-grid">
              ...
              <label class="field" id="entity-appeared-field"><span id="entity-appeared-label">First appearance chapter (optional)</span><input name="appeared" type="number" min="1" placeholder="Leave blank if they have not appeared yet"></label><label class="field" id="entity-fades-field"><span>Stays on the graph for (optional)</span><input name="fades" type="text" inputmode="numeric" maxlength="3" placeholder="Leave blank to stay for good"><small class="field-note">How many actions this keeps standing on the graph after the last one it is in. Write 0 for somebody who walks on for one scene and is gone the moment it ends. Leave it empty and it stays for good, as everything does now.</small></label>
...
function passingLinger(id){const linger=Number(entity(id)?.fades);return Number.isFinite(linger)&&linger>=0?linger:null;}
function passingWatch(){
  const watched=data.entities.filter(item=>passingLinger(item.id)!==null).map(item=>item.id);
  const lastSeen=new Map();
  if(watched.length)volumeActions().forEach((event,index)=>watched.forEach(id=>{if(eventInvolves(event,id))lastSeen.set(id,index+1);}));
  return lastSeen;
}
// Gone for good from here on, and separately: still here, but on its way out.
function passingGone(id,lastSeen){const linger=passingLinger(id),at=lastSeen.get(id);return linger!==null&&at!==undefined&&currentActionIndex>at+linger;}
function passingLeaving(id,lastSeen){const linger=passingLinger(id),at=lastSeen.get(id);return linger!==null&&at!==undefined&&currentActionIndex>at&&currentActionIndex<=at+linger;}
