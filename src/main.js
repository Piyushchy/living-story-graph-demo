...
2 | https://www.webnovel.com/book/.../chapter-title_789012"></textarea></label>
          ...
          <div class="admin-grid">
            <form id="entity-form" class="admin-card"><div class="admin-card-heading"><h2 id="entity-form-title">Create character, organization, or location</h2><div class="manage-row"><input id="manage-entity" list="admin-entity-options" placeholder="Load an existing identity"><button class="button ghost" id="load-entity" type="button">Load</button></div></div><p id="creation-edit-note" class="event-edit-mode-note" hidden><strong>Editing a creation-generated row.</strong> These facts were made by this identity form, so saving here updates the original linked row instead of creating a chapter event.</p><input name="editingId" type="hidden"><input name="editingCreationEventId" type="hidden"><div class="form-grid">
              ...
              <label class="field" id="entity-appeared-field"><span id="entity-appeared-label">First appearance chapter (optional)</span><input name="appeared" type="number" min="1" placeholder="Leave blank if they have not appeared yet"></label><label class="field" id="entity-fades-field"><span>Stays on the graph for (optional)</span><div class="fades-row"><input name="fades" type="text" inputmode="numeric" maxlength="3" placeholder="Leave blank to stay for good"><select name="fadesUnit"><option value="chapter">more chapters</option><option value="action">more actions</option></select></div><small class="field-note">How much longer this keeps standing on the graph after the last action it is in. <strong>0 more chapters</strong> is the one for a face that belongs to a single chapter — five people in a classroom, a crowd at a gate: they go when the chapter turns. Leave it empty and it stays for good, as everything does now.</small></label>
...
// A shopkeeper in one scene, a carriage nobody boards twice: some identities walk on, do their
// one thing, and are never seen again. Left on the graph for ever they are clutter. A passing
// identity carries how many actions it stays for after the last one it is in — nought means it
// goes as soon as its scene is over — and it leaves once that many have gone by.
// Counted in actions, this is the same unit the "lasts for" counter on an action uses. Counted in
// chapters, it is the unit a writer actually thinks in for a walk-on: five faces in one classroom
// belong to that chapter and to no other, and nobody should have to count the actions in it.
function passingLinger(id){const item=entity(id),linger=Number(item?.fades);return Number.isFinite(linger)&&linger>=0?{linger,byChapter:item?.fadesUnit==="chapter"}:null;}
function passingWatch(){
  const watched=data.entities.filter(item=>passingLinger(item.id)!==null).map(item=>item.id);
  const lastSeen=new Map();
  if(watched.length)volumeActions().forEach((event,index)=>watched.forEach(id=>{if(eventInvolves(event,id))lastSeen.set(id,{at:index+1,chapter:event.chapter});}));
  return lastSeen;
}
function passingSpan(id,lastSeen){
  const rule=passingLinger(id),seen=lastSeen.get(id);
  if(!rule||!seen)return null;
  return rule.byChapter
    ?{now:currentChapter,from:seen.chapter,end:seen.chapter+rule.linger}
    :{now:currentActionIndex,from:seen.at,end:seen.at+rule.linger};
}
// Gone for good from here on, and separately: still here, but on its way out.
function passingGone(id,lastSeen){const span=passingSpan(id,lastSeen);return Boolean(span&&span.now>span.end);}
function passingLeaving(id,lastSeen){const span=passingSpan(id,lastSeen);return Boolean(span&&span.now>span.from&&span.now<=span.end);}
