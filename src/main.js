...
const sampleData = {
  ...
  entities: [
    ...
    { id: "q-winter", kind: "quest", issuer: "inn-system", name: "Survive the Long Frost", intro: 18, countsAlone: true, questBadge: "Urgent", timeLimit: "Ninety days", failure: "Authority falls by one and the Hearthkeeper System is recalled.", description: "Keep every guest alive until the thaw." },
...
function bindEventPanelRows(){
  ...
  document.querySelectorAll("[data-focus-action]").forEach(row=>row.onclick=event=>{if(event.target.closest("button,a,input,textarea"))return;cancelChapterSequence();currentActionIndex=Number(row.dataset.focusAction);currentChapter=currentActionEvent()?.chapter||activeVol().from;renderAll();});}
