...
function actionHaystack(event){
  ...
  const carried=[event.rewards?.length?`rewards reward ${event.rewards.join(" ")}`:"",String(event.type)==="quest_end"&&event.value?`rewards reward ${event.value}`:"",event.performance?`performance ${event.performance}`:"",event.grade?`grade ${event.grade}`:"",event.authority?`authority ${event.authority}`:""].filter(Boolean).join(" ");
